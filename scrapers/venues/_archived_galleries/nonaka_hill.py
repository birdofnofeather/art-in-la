"""Nonaka-Hill — Artlogic h2 block text, Los Angeles filter."""
...
BASE = "https://nonaka-hill.com"
_DATE_RE = re.compile(
    r"(\d{1,2}\s+(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?"
    r"|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)"
    r"(?:\s+\d{4})?\s*[-–—]\s*(?:\d{1,2}\s+)?(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?"
    r"|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?"
    r"|Nov(?:ember)?|Dec(?:ember)?)?\s*\d{1,2},?\s*\d{4}"
    r"|(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?"
    r"|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)"
    r"\s+\d{1,2}(?:,\s*\d{4})?\s*[-–—]\s*(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?"
    r"|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?"
...
class Scraper(BaseScraper):
    venue_id = "nonaka_hill"
    events_url = f"{BASE}/exhibitions"
    source_label = "nonaka-hill.com"
    ...
    def custom_parse(self,html,base_url):
        ...
        for h2 in soup.find_all("h2"):
            parent=h2.find_parent(["div","li","article","section"])
            text=parent.get_text(" ",strip=True) if parent else h2.get_text(" ",strip=True)
            if "los angeles" not in text.lower(): continue
            m=_DATE_RE.search(text)
            if not m: continue
            title=_DATE_RE.sub("",h2.get_text(" ",strip=True)).strip().rstrip("-–").strip()
            if not title or title in seen or len(title)<3: continue
            seen.add(title)
            start,end=_pr(m.group(0))
            a=(parent or h2).find("a",href=True)
