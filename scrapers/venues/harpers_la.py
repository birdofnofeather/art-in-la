"""Harper's Gallery — Artlogic CMS div.entry h1/h2/h3. venue_id=harpers"""
...
BASE = "https://harpersgallery.com"
_YEAR_RE = re.compile(r"\b(20\d\d)\b")
_CUR_YEAR = datetime.now().year
def _pr(s):
    s=s.strip().replace("–","-").replace("—","-")
    parts=re.split(r"\s+-\s+",s,maxsplit=1)
    ym=_YEAR_RE.search(s); yr=int(ym.group(1)) if ym else _CUR_YEAR; d=datetime(yr,1,1)
    try:
        if len(parts)==1: return dparser.parse(parts[0],default=d).strftime("%Y-%m-%dT00:00:00-07:00"),None
...
class Scraper(BaseScraper):
    ...
    source_label = "harpersgallery.com"
